...
print("loading model...")
model = load_model(device="cuda:1", full_precision=False)
...
def get_examples():
    ...
    for item in examples:
        if item.is_file():
            ...
            results.append([[str(p) for p in item.glob("*.png")]])
    return results
...
def run_inference(image_inputs: list[Image.Image]):
    imgs = [tup[0] for tup in image_inputs]
...
iface = gr.Interface(
    run_inference,
    inputs=[gr.Gallery(label="上传页面图像", type="pil")],
    outputs=[
        gr.Markdown(
            label="预测结果",
            latex_delimiters=[
